...
{
    float32_t3 scatteringColor = baseTexColor.rgb*baseTexColor.rgb; 
    ...
    // 散乱
    float32_t3 scatteredColor = scatteringColor * (1.0 - attenuation);
    return float32_t3((1.0 - attenuation) * (absorbedColor + scatteredColor));
}
...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    for (int32_t i = 0; i < gNowLightTotal.count; i++)
    {
        ...
        //Rim
        float32_t3 RimColor = RimLight(pLightDir, toEye, 100.0f, gPointLight[i].intensity, gPointLight[i].color.rgb) * Light_Intensity_factor;
        //SSS
        float32_t3 SSSColor = ComputeSubsurfaceScattering(baseTexColor.rgb, N, toEye, pLightDir) * Light_Intensity_factor;
		//拡散
